 ...
 */
package de.thkoeln.fentwums.fsm.generation;

import de.thkoeln.fentwums.fsm.generation.Lexer.TOKEN;
import de.thkoeln.fentwums.fsm.generation.OutputVectorParser.GeneratedOutputVector;
import de.thkoeln.fentwums.fsm.generation.VariableAssignmentParser.GeneratedVarAssignment;
import de.thkoeln.fentwums.fsm.graph.Component;
import de.thkoeln.fentwums.fsm.graph.Graph;
import de.thkoeln.fentwums.fsm.graph.Graph.GRAPH_TYPE;
import de.thkoeln.fentwums.fsm.graph.SigVar.SIGVAR_TYPE;
import de.thkoeln.fentwums.fsm.graph.Signal;
import de.thkoeln.fentwums.fsm.graph.Signal.SIGNAL_DIRECTION;
import de.thkoeln.fentwums.fsm.graph.State;
import de.thkoeln.fentwums.fsm.graph.Transition;
import de.thkoeln.fentwums.fsm.graph.Variable;
...
public class Generation implements I_GENERATION {
    ...
    // *** METHODS ***
    /**
     * Checks the integrity and determinism of the current graph
     *
     * @param graph graph that shall be verified
     * @return success of intergrity and determinism check
     *
    ...
    public String checkIntegrityAndDeterminism(Graph graph) {
        ...
        // get the list of all input and inOut Signals
        LinkedList<Signal> inputSignals = graph.getSignals(Signal.SIGNAL_DIRECTION.IN);
        inputSignals.addAll(graph.getSignals(Signal.SIGNAL_DIRECTION.INOUT));

        // split List into signals of type BIT and others (handled as integer)
        LinkedList<String> bitSignals = new LinkedList();
        LinkedList<String> intSignals = new LinkedList();
        for (Signal signal : inputSignals) {
            if (signal.getType() == SIGVAR_TYPE.BIT) {
                bitSignals.add(signal.getName());
            } else {
                intSignals.add(signal.getName());
            }
        }
        // add all constants (e.g. 0x07 or #5) used in the graph to the list of int Signals
        LinkedList<String> constantsUsed = getConstants(graph);
        intSignals.addAll(constantsUsed);
        
        // add all variables used in the graph to the list of int / bit signals
        LinkedList<Variable> varList = graph.getVariables();
        for (Variable variable : varList){
            if(variable.getType() == SIGVAR_TYPE.BIT){
                bitSignals.add(variable.getName());
            } else {
                intSignals.add(variable.getName());
            }
        }

        // set up a datastructure that includes all combinations of input signal values - for BIT there are 2 combinations [0, 1] 
        // for non-BIT we'll use n(intSignals)^n(intSignals) combinations for code simplification. 
        // don't continue if amount of non-BIT signals exceeds 7 due to expected 
        // high run time O(n(intSignals)^n(intSignals)*2^n(bitSignals)*n(States)*n(Transitions))
        if (intSignals.size() > 7) {
            errorLog.append("Zu viele Input-Signale des Typs "
                    + "\n BIT_N, UNSIGNED oder SIGNED oder Konstanten gewählt"
                    + "\n erwartete Laufzeit ist zu hoch!");
            return errorLog.getLogString();
        }

        // all test values that are required to check for integrity and determinism
        LinkedList<ConcurrentHashMap<String, Integer>> allInputVectors = new LinkedList<ConcurrentHashMap<String, Integer>>();

        // add all int values to the input Vectors
        if (!intSignals.isEmpty()) {
            allInputVectors = addIntVectors(intSignals);
        } else {
            // add empty map for following function call
            allInputVectors.add(new ConcurrentHashMap<String, Integer>());
        }
        long expectedInputVectors = (long) Math.pow(2, bitSignals.size()) * allInputVectors.size();
        // don't continue if amount of BIT signals * 2 * amount of current inputVectors exceeds 1million
        if (expectedInputVectors > 1000000) {
            errorLog.append(" Anzahl zu prüfender Werte (" + expectedInputVectors + ")"
                    + " \n und dadurch erwartete Prüfdauer sind zu hoch.");
            return errorLog.getLogString();
        }

        //  add all bit values to the input Vectors
        if (!bitSignals.isEmpty()) {
            allInputVectors = addBitVectors(bitSignals, allInputVectors);
        }

        /*  Check for every state how many conditions are met by
            any of the input Vectors.
            If at least one condition is met, the graph is complete (integrity is confirmed)
            If more than one condition is met, the graph is not deterministic
            
         */
        LinkedList<State> stateList = graph.getStates();
        boolean isComplete = true;
        boolean isDeterministic = true;
        for (State state : stateList) {
            boolean stateIsComplete = true;
            boolean stateIsDeterministic = true;
            for (ConcurrentHashMap<String, Integer> inputVector : allInputVectors) {
                int conditionsMet = 0;
                for (Transition transition : state.getTransitions()) {
                    Expression expression = new Expression(transition.getCondition());
       ...
       /* if (isComplete) {
        ...
        if (isDeterministic) {
            ...
            errorLog.append(">> Der Graph is NICHT deterministisch! <<");
        }
        return errorLog.getLogString();
    }

    /**
     * Retrieves the names of all constants used in conditions in the graph
     *
     * @param graph graph that shall be searched
     * @return list of constant names
     *
     * @author Markus de Koster
     */
    private LinkedList<String> getConstants(Graph graph) {
        LinkedList<String> usedConstants = new LinkedList<String>();
        for (State state : graph.getStates()) {
            for (Transition transition : state.getTransitions()) {
                Lexer lexer = new Lexer();
                lexer.setString((transition.getCondition() + '\0').toCharArray());
                while (lexer.getToken() != TOKEN.END) {
                    String number = Integer.toString(lexer.getNumber());
                    if ((lexer.getToken() == TOKEN.NUMBER)
                            && !usedConstants.contains(number)) {
                        usedConstants.add(number);
                    }
                    lexer.getNextToken();
                }
            }
    ...
    /**
     * add all int values to the input Vectors For a complete integrity /
     * determinism check n(intSignals)^n(intSignals) input Vectors are required.
     * E.g. for 3 signals we require 27 input Vectors Example: Signal A,B,C each
     * have values [0,1,2] Thus all combinations of [0,1,2] for the 3 signals
     * are required This way there are test values for each comparison operator
     * [<,>,<=,>=,=,!=] constants are handled the same way as int signals e.g.
     * #342 will be handled as signal with the name 342
     *
     ...
     * @author Markus de Koster
     */
    private LinkedList<ConcurrentHashMap<String, Integer>> addIntVectors(LinkedList<String> intSignals) {
        LinkedList<ConcurrentHashMap<String, Integer>> allInputVectors = new LinkedList<ConcurrentHashMap<String, Integer>>();
        ConcurrentHashMap<String, Integer> inputVector = new ConcurrentHashMap<String, Integer>();
        // add first element to the list as starting point for following code sequence
        allInputVectors.add(inputVector);
        for (String sigName : intSignals) {
            // ListIterator is being used to modfiy the list while iterating through it
            ListIterator<ConcurrentHashMap<String, Integer>> iterator = allInputVectors.listIterator();
            while (iterator.hasNext()) {
                inputVector = iterator.next();
                iterator.remove();
                for (int i = 0; i < intSignals.size(); i++) {
                    ConcurrentHashMap<String, Integer> newInputVector = new ConcurrentHashMap<String, Integer>();
                    newInputVector.putAll(inputVector);
                    newInputVector.put(sigName, i);
                    iterator.add(newInputVector);
                }
            }
        }
        return allInputVectors;
    }

    /**
     * add all bit values to the input Vectors Bit values can either be true or
     * false, represented by 1 and 0 For a complete integrity and determinism
     * check all combinations of Bit values have to be added to the current list
     ...
     * @author Markus de Koster
     */
    private LinkedList<ConcurrentHashMap<String, Integer>> addBitVectors(LinkedList<String> bitSignals, LinkedList<ConcurrentHashMap<String, Integer>> allInputVectors) {
        // Mapping of Signal Name to Value equals one set of test values (= input Vector)
        ConcurrentHashMap<String, Integer> inputVector;
        for (String sigName : bitSignals) {
            // ListIterator is being used to modfiy the list while iterating through it
            ListIterator<ConcurrentHashMap<String, Integer>> iterator = allInputVectors.listIterator();
            while (iterator.hasNext()) {
                inputVector = iterator.next();
                ...
                newInputVector.putAll(inputVector);
                iterator.remove();
                inputVector.put(sigName, 0); // boolean false
                iterator.add(inputVector);
                newInputVector.put(sigName, 1); // boolean true
                iterator.add(newInputVector);
            }
        }
        return allInputVectors;
     ...
     *
     * @author Andreas Schwenk, Markus de Koster
     */
    @Override
...
// END DEBUG
        ...
        while (itStates.hasNext()) {
            ...
            for (Iterator itTrans = state.getTransitions().iterator(); itTrans.hasNext();) {
                ...
                //write position
                Point condPos = transition.getConditionPosition();
                pw.println("          <conditionPosition x=\"" + condPos.x + "\" y=\"" + condPos.y + "\"/>");

                //write startPoint
                Point startPoint = transition.getFromStateDockingPoint();
                pw.println("          <startPoint x=\"" + startPoint.x + "\" y=\"" + startPoint.y + "\"/>");

                //write endPoint
                Point endPoint = transition.getToStateDockingPoint();
                pw.println("          <endPoint x=\"" + endPoint.x + "\" y=\"" + endPoint.y + "\"/>");

                //write all Controlpoints to file
                LinkedList<Point> ctrlPoints = transition.getCtrlPoints();
                pw.println("          <ctrlPoints>");
                for (Point p : ctrlPoints) {
                    pw.println("            <ctrlPoint x=\"" + p.x + "\" y=\"" + p.y + "\"/>");
                }
                pw.println("          </ctrlPoints>");
        ...
        Point condPos = startNode.getConditionPosition();
        pw.println("    <conditionPosition x=\"" + condPos.x + "\" y=\"" + condPos.y + "\"/>");
        Point startNodeStartPos = startNode.getStartNodeStartPos();
        pw.println("    <position x=\"" + startNodeStartPos.x + "\" y=\"" + startNodeStartPos.y + "\"/>");
        Point endPoint = startNode.getToStateDockingPoint();
        pw.println("    <targetPosition x=\"" + endPoint.x + "\" y=\"" + endPoint.x + "\"/>");
